...
3. **QA template:** `{{QA_TEMPLATE}}` — the exact template you must follow.
...
Compose the QA plan at `{{QA_FILE}}` per the QA template. Split items into:
